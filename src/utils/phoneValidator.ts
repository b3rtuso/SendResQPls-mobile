...
export const validatePhilippineMobile = (phone: string | undefined | null): PhoneValidationResult => {
  ...
  if (raw.startsWith('+63') || raw.startsWith('+')) {
    return {
      valid: false,
      error: 'Please remove +63. Enter an 11-digit mobile number starting with 09 (e.g. 09123456789).'
    };
  }
  ...
  if (raw.length < 11) {
    return {
      valid: false,
      error: `Mobile number is incomplete (kulang) with ${raw.length} digits. It must be exactly 11 digits starting with 09 (e.g. 09123456789).`
    };
  }
  ...
  if (raw.length > 11) {
    return {
      valid: false,
      error: `Mobile number exceeds 11 digits (${raw.length} digits entered). It must be exactly 11 digits starting with 09 (e.g. 09123456789).`
    };
  }
  ...
  if (!raw.startsWith('09')) {
    return {
      valid: false,
      error: 'Mobile number must start with 09 (e.g. 09123456789).'
    };
  }
